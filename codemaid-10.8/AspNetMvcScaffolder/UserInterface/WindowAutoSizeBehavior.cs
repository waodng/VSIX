...
namespace AspNetMvcScaffolder.UserInterface
{
    ...
    {
        public static readonly DependencyProperty IsEnabledProperty = DependencyProperty.RegisterAttached("IsEnabled", typeof(bool), typeof(WindowAutoSizeBehavior), new PropertyMetadata(OnIsEnabledChanged));
        ...
        private static void Window_ContentRendered(object sender, EventArgs e)
        {
            ...
            if (window.SizeToContent.HasFlag(SizeToContent.Height) && DependencyPropertyHelper.GetValueSource(window, FrameworkElement.MinHeightProperty).BaseValueSource != BaseValueSource.Local)
            {
                window.MinHeight = window.ActualHeight;
            }
            if (window.SizeToContent.HasFlag(SizeToContent.Height) && DependencyPropertyHelper.GetValueSource(window, FrameworkElement.MaxHeightProperty).BaseValueSource != BaseValueSource.Local)
            {
                window.MaxHeight = window.ActualHeight;
            }

            window.Width = window.ActualWidth;
            if (window.SizeToContent.HasFlag(SizeToContent.Width) && DependencyPropertyHelper.GetValueSource(window, FrameworkElement.MinWidthProperty).BaseValueSource != BaseValueSource.Local)
            {
