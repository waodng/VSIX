...
namespace AspNetMvcScaffolder.VisualStudio
{
    internal class EditorIntegration : IEditorIntegration
    {
        ...
        public void FormatDocument(string filePath)
        {
            ...
            OLECMD[] commandStatus = new OLECMD[] { new OLECMD() { cmdID = (uint)VSConstants.VSStd2KCmdID.FORMATDOCUMENT } };
        ...
        public IEditorInterfaces GetOrOpenDocument(string path)
        {
            ...
            try
            {
                int hr = runningDocumentTable.FindAndLockDocument((uint)_VSRDTFLAGS.RDT_NoLock, path, out hierarchy, out itemId, out documentData, out cookie);
        ...
        public IVsWindowFrame CreateAndOpenReadme(string text)
        {
            ...
            int hr = openDocument.OpenDocumentViaProject(tempFilename, ref logicalView, out oleServiceProvider, out hierarchy, out itemId, out window);
        ...
        private class NotificationActivator : IDisposable
        {
            ...
            public void Dispose()
            {
                editorIntegration.ResumeChangeNotifications(filePath);
            }
        }
