...
namespace AspNetMvcScaffolder
{
    internal static class ScaffolderFilter
    {
        ...
        {
            return DisplayScaffolders(codeGenerationContext, AssemblyVersions.WebApiAssemblyName, AssemblyVersions.WebApiAssemblyMinVersion, AssemblyVersions.WebApiAssemblyMaxVersion);
        }

        /// <summary>
        /// Verifies whether the project is a CSharp project and a supported version or no version at all is installed of OData packages are installed.
        /// </summary>
        public static bool DisplayODataScaffolders(CodeGenerationContext codeGenerationContext)
        {
            return DisplayScaffolders(codeGenerationContext, AssemblyVersions.ODataAssemblyName, AssemblyVersions.ODataAssemblyMinVersion, AssemblyVersions.ODataAssemblyMaxVersion);
        }
        ...
        private static bool IsApplicableProject(CodeGenerationContext context)
        {
            ...
            if (ProjectLanguage.CSharp == context.ActiveProject.GetCodeLanguage() ||
                ProjectLanguage.VisualBasic == context.ActiveProject.GetCodeLanguage())
            {
            ...
            string projectReferenceName, Version minVersion, Version maxExcludedVersion)
        {
            ...
            foreach (string assemblyPath in projectReferences)
            {
                ...
                {
                    if (assemblyName.Version.Major >= minVersion.Major &&
                        assemblyName.Version.Major < maxExcludedVersion.Major)
                    {
