...
namespace AspNetMvcScaffolder
{
    internal class PackageVersions
    {
        ...
        private static readonly Version _latestKnownPackageVersion = new Version(5, 2, 2);
        ...
        private static string GetPackageReferenceFileName(CodeGenerationContext context)
        {
            ...
            // When we don't have any package references / assembly references, we should
            // use the package file with latest package versions.
            string packageFileName = GetPackageVersionsFileName(_latestKnownPackageVersion);

            // See if the project has a reference to MVC and then
            // use the package file with package versions based on the
            // current MVC package / assembly reference.

            //GetPackageFileNameForPackage(context, installedPackages,
            //    packageId: NuGetPackages.MvcNuGetPackageId,
            //    assemblyReferenceName: AssemblyVersions.MvcAssemblyName,
            //    minSupportedAssemblyReferenceVersion: AssemblyVersions.MvcAssemblyMinVersion,
            //    packageFileName: ref packageFileName);

            // If the project has reference to a different version of Web API, 
            // the package version file corresponding to the Web API version is loaded
            // because OData has a direct dependency on the Web API version.
            // Hence, Web API version takes precedence over the MVC version.
            GetPackageFileNameForPackage(context, installedPackages,
        ...
        {
            return String.Format(CultureInfo.InvariantCulture, PackageVersionsFile, version.Major + "." + version.Minor + "." + version.Build);
        }

        internal static IEnumerable<IVsPackageMetadata> GetInstalledPackages(CodeGenerationContext context)
        {
            var packageInstallerServices = context.ServiceProvider.GetService<IComponentModel, SComponentModel>().GetService<IVsPackageInstallerServices>();
