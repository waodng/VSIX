...
namespace SteveCadwallader.CodeMaid.Helpers
{
    ...
    internal static class TextDocumentHelper
    {
        ...
        internal static void CustomAllStringMatches(TextDocument textDocument, string patternString, string replacementString)
        {
            
            //TextRanges dummy = null;
            //int lastCount = -1;
            //#region vsFindOptions选项和注释
            //        /*
            //        vsFindOptionsBackwards	从当前点向后搜索。
            //        vsFindOptionsFromStart	从文档的开头开始搜索。
            //        vsFindOptionsKeepModifiedDocumentsOpen	不关闭搜索修改过的文档。
            //        vsFindOptionsMatchCase	在搜索中匹配单词大小写。
            //        vsFindOptionsMatchInHiddenText	在隐藏文本和非隐藏文本中查找匹配的文本。由于 FindPattern 方法搜索所有文本（包括隐藏文本），
            //                                         * 因此 vsFindOptionsMatchInHiddenText 常数值不适用于此方法。
            //        vsFindOptionsMatchWholeWord	在搜索中进行全字匹配。
            //        vsFindOptionsNone	无匹配。
            //        vsFindOptionsRegularExpression	匹配正则表达式。
            //        vsFindOptionsSearchSubfolders	在单词搜索中包括子文件夹。
            //        vsFindOptionsWildcards	在字符串搜索中使用通配符。
            //         * */

            //#endregion
            //const int customFindOptions = (int)(vsFindOptions.vsFindOptionsRegularExpression | 
            //                                           vsFindOptions.vsFindOptionsMatchInHiddenText);
            //while (textDocument.ReplacePattern(patternString, replacementString, customFindOptions, ref dummy))
            //{
            //    // it is possible that the replacements aren't actually being done. In such a case,
            //    // we can detect the situation by seeing if the count always remains the same, and
            //    // if so exiting early.
            //    if (lastCount == dummy.Count)
            //    {
            //        OutputWindowHelper.WarningWriteLine("Forced a break out of TextDocumentHelper's CustomAllStringMatches for a document.");
            //        break;
            //    }
            //    lastCount = dummy.Count;
            //}
